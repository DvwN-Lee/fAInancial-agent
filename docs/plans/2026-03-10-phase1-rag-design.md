...
scripts/index_documents.py (CLI)
  ...
  ├─ Voyage AI voyage-3.5-lite 배치 임베딩
  │   ※ 무료 200M 토큰 (신용카드 불필요). 배치 인덱싱 ~87만 토큰 = 여유
...
| `voyageai` | 쿼리 임베딩 (Voyage AI voyage-3.5-lite) |
...
| `faiss-cpu` | 인덱스 생성 |
| `voyageai` | 문서 임베딩 (Voyage AI voyage-3.5-lite) |
| `ragas` | RAG 품질 평가 |
...
| 임베딩 | Voyage AI voyage-3.5-lite | 무료 200M 토큰 (신용카드 불필요). 한국어 지원. Gemini 1,000 RPD 제한 회피 |
...
Voyage AI voyage-3.5-lite는 200M 무료 토큰(신용카드 불필요), 한국어 지원.

**변경 범위**:

| 항목 | Before (Gemini) | After (Voyage AI) |
|------|----------------|-------------------|
| 임베딩 모델 | text-embedding-004 | voyage-3.5-lite |
| 무료 한도 | 1,000 RPD | 200M 토큰 |
...
- FAISS 인덱스 차원: 768 → 1024 (voyage-3.5-lite 기본값). 기존 인덱스 재생성 필수
